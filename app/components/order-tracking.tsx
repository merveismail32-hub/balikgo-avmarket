...
const steps: { status: Exclude<OrderStatus, "CANCELLED">; label: string }[] = [
  ...
  { status: "READY_TO_SHIP", label: "Kargoya Hazır" },
...
export function OrderTracking({ statuses }: { statuses: OrderStatus[] }) {
  ...
  const current = Math.max(...statuses.map((status) => status === "COMPLETED" ? steps.length - 1 : Math.max(0, steps.findIndex((step) => step.status === status))));
