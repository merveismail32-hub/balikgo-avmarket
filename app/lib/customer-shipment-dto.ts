...
export function toCustomerOrderDto(order: CustomerOrderRecord) {
  return {
    ...
    shipments: order.shipments.map(({ events, ...shipment }) => ({
      ...
      timeline: events.filter((event) => event.applied).map((event) => ({ id: event.id, status: canonicalShipmentStatus(event.status), label: shipmentStatusLabel(event.status), eventTime: event.eventTime, receivedAt: event.receivedAt, location: event.location, description: event.description })),
    })),
  };
}
