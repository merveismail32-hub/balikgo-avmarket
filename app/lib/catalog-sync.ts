...
export async function ensureCatalogForProduct(tx: Prisma.TransactionClient, productId: string) {
  ...
  const offer = product.sellerOffer ?? await tx.sellerOffer.create({ data: { sellerId: product.sellerId, catalogProductId: catalog.id, legacyProductId: product.id, sellerSku: product.sku, price: product.price, listPrice: product.oldPrice, stock: product.stock, active: product.active, createdAt: product.createdAt } });
