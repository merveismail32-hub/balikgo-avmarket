...
export default async function EditSellerProductPage({ params }: PageProps<"/satici-panel/urunler/[id]/duzenle">) {
  ...
  const [product, categories, brands] = await Promise.all([
    prisma.product.findFirst({ where: { id, sellerId: seller.id }, include: { sellerOffer: { select: { stock: true, inventoryVersion: true, priceVersion: true } } } }),
  ...
  return <SellerPanelShell title="Ürünü düzenle" description="Ürün bilgilerini, stok durumunu ve görselleri güncelleyin."><SellerProductEditForm categories={categories} brands={brands} product={{ id: product.id, inventoryVersion: product.sellerOffer?.inventoryVersion ?? 0, priceVersion: product.sellerOffer?.priceVersion ?? 1, name: product.name, sku: product.sku, moderationStatus: product.moderationStatus, moderationReason: product.moderationReason, categoryId: product.categoryId, brandId: product.brandId, category: product.category, brand: product.brand, price: Number(product.price), oldPrice: product.oldPrice ? Number(product.oldPrice) : null, stock: product.sellerOffer?.stock ?? 0, description: product.description, technicalDetails: product.technicalDetails, shippingInfo: product.shippingInfo, active: product.active, images: initialImages }} /></SellerPanelShell>;
}
