...
export async function POST(request: Request) {
  ...
  try {
    const preview = await prisma.$transaction(async (tx) => {
      const cart = await tx.cartItem.findMany({ where: { userId: session.user.id, product: publicProductPolicy }, select: { productId: true, quantity: true, product: { select: { sellerId: true, price: true, stock: true } } } });
      if (!cart.length) throw new Error("Sepetiniz boş."); if (cart.some((line) => line.quantity > line.product.stock)) throw new Error("Sepetinizde stok miktarı değişen ürün var.");
      const subtotal = cart.reduce((sum, line) => sum.add(line.product.price.mul(line.quantity)), new Prisma.Decimal(0)); const result = await evaluateCoupon(tx, { code: parsed.data.couponCode, userId: session.user.id, lines: cart });
