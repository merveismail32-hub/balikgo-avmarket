...
async function responseFor(userId: string) {
  const items = await prisma.cartItem.findMany({ where: { userId }, include: { product: { include: { seller: true } } }, orderBy: { createdAt: "asc" } });
...
export async function POST(request: Request) {
  ...
  for (const item of items) {
    ...
    const product = await prisma.product.findFirst({ where: { id: value.id, active: true }, select: { stock: true } });
...
export async function PATCH(request: Request) {
  ...
  const product = await prisma.product.findFirst({ where: { id: body.productId, active: true }, select: { stock: true } });
