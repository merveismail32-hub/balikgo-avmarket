...
async function responseFor(userId: string) {
  const favorites = await prisma.favorite.findMany({ where: { userId }, include: { product: true }, orderBy: { createdAt: "asc" } });
...
export async function POST(request: Request) {
  ...
  const productIds = Array.isArray(body) ? body.filter((id): id is string => typeof id === "string") : [];
  for (const productId of productIds) await prisma.favorite.upsert({ where: { userId_productId: { userId, productId } }, create: { userId, productId }, update: {} });
